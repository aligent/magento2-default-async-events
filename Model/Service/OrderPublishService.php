...
class OrderPublishService
{
    ...
    /**
     * Publishes an order event if enabled
     *
     * @param OrderInterface $order
     * @param string $eventName
     * @return void
     */
    public function execute(OrderInterface $order, string $eventName): void
    {
        if ($this->configHelper->canPublishOrder($order)) {
            $this->publisher->publish(
                QueueMetadataInterface::EVENT_QUEUE,
                [
                    $eventName,
                    $this->serializer->serialize(['id' => $order->getEntityId()])
                ]
            );
        }
    }
